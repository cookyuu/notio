...
- [ ] `최근 5시간 내의 알림 내역을 요약해줘` 입력이 UI에서 잘리지 않고 전송되는지 확인한다.
- [ ] `지난 30분 동안 중요한 알림 알려줘` 입력이 기존 전송 버튼/엔터 동작으로 전송되는지 확인한다.
- [ ] `오늘 받은 알림 요약해줘` 입력이 기존 chat bubble에 정상 표시되는지 확인한다.
- [ ] `어제 GitHub 알림 정리해줘` 입력이 기존 chat bubble에 정상 표시되는지 확인한다.
- [ ] 기간 표현을 위한 별도 필터 UI를 추가하지 않는다.
- [ ] 미지원 기간 표현을 입력해도 프론트에서는 일반 질문과 동일하게 처리한다.
