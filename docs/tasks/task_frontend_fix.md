...
- [ ] 상세 조회 후 목록의 unread indicator가 즉시 사라지도록 상태 갱신 로직을 추가한다.
- [ ] 상세 조회 후 목록 title style이 read 상태로 즉시 반영되도록 확인한다.
- [ ] 상단 unread badge가 상세 조회 후 감소하도록 확인한다.
- [ ] pull-to-refresh 이후 서버의 읽음 상태와 목록 상태가 일치하는지 확인한다.
- [ ] 이번 범위에서 상세 데이터 로컬 캐시를 유지할지 제외할지 구현 기준을 명확히 한다.
