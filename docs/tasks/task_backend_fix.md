...
- [ ] 상세 조회 전용 service 메서드를 명확히 분리한다.
- [ ] 상세 조회 시 대상 알림이 미읽음이면 읽음 상태로 전환한 뒤 반환하도록 구현한다.
- [ ] 이미 읽은 알림이면 상태 변경 없이 상세 데이터만 반환하도록 구현한다.
- [ ] 실제 읽음 전환이 발생한 경우 unread count cache eviction이 보장되도록 정리한다.
- [ ] 상세 조회 로직이 별도 `PATCH /notifications/{id}/read` 계약과 충돌하지 않도록 정리한다.
