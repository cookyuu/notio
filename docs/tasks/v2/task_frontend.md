...
- [x] Deliveries 탭 메시지 탭 → `NotificationDetailModal` 바텀 시트 정상 표시
- [x] 로딩 중 동일 항목 중복 탭 → 모달이 두 번 열리지 않는지 확인
- [x] 네트워크 오류 상황 → 스낵바 에러 메시지 표시 확인
- [x] 모달 닫기(드래그 다운 / 닫기 버튼) 정상 동작 확인
