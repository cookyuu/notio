...
- [ ] `buildNotificationSummaryPrompt(Notification, List<RagDocument>)` 메서드 추가
  - [ ] 시스템 프롬프트: 2~4문장, 500자 제한, 마크다운 활용 규칙 포함
  - [ ] 유저 프롬프트: 소스, 제목, 우선순위, 내용, 링크(nullable) 포함
  - [ ] RAG context가 있을 경우 유사 과거 알림 top-3 포함
  - [ ] `LlmPrompt.of(systemPrompt, userPrompt)` 반환

### 2-3. buildDigestSummaryPrompt 추가

- [ ] `buildDigestSummaryPrompt(List<Notification>)` 메서드 추가
  - [ ] 시스템 프롬프트: 첫 줄 전체 요약, 항목별 1줄 요약, 1000자 제한 규칙 포함
  - [ ] 유저 프롬프트: 알림 개수 표시, 각 알림 소스/제목/우선순위/내용 포함
  - [ ] `n.getAiSummary() != null` 이면 aiSummary, 아니면 body 사용
  - [ ] `truncate(String, int)` private 헬퍼 메서드 추가 (최대 300자)
  - [ ] `LlmPrompt.of(systemPrompt, userPrompt)` 반환
