...
@Component
public class CodexConnectionAdapter implements ConnectionProviderAdapter {

    private final WebhookCredentialAuthenticationService authenticationService;

    public CodexConnectionAdapter(final WebhookCredentialAuthenticationService authenticationService) {
        this.authenticationService = authenticationService;
    }

    @Override
    public ConnectionProvider supports() {
        return ConnectionProvider.CODEX;
    }
    ...
    public WebhookPrincipal authenticateWebhook(final WebhookRequestContext context) {
        return authenticationService.authenticateApiKey(context.headers(), ConnectionProvider.CODEX);
    }
    ...
    public NotificationEvent toNotificationEvent(final WebhookRequestContext context) {
        ...
        return new NotificationEvent(
            NotificationSource.CODEX,
            firstNonBlank(WebhookPayloadExtractor.stringValue(payload, "title", "notification.title"), "Codex notification"),
